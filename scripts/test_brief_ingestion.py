...
class IngestionTests(unittest.TestCase):
    def test_appends_without_rewriting_existing_event(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp); first=event(); (root/"2026-09-16.json").write_text(json.dumps([first]),encoding="utf-8")
            second=event("EVT-20260916-0002","2026-09-16T20:00:00+00:00")
            with patch.object(mod,"DATA_DIR",root): mod.ingest([second])
            saved=json.loads((root/"2026-09-16.json").read_text(encoding="utf-8")); self.assertEqual(saved,[first,second])
    ...
    def test_rejects_invalid_event_before_write(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((root/"2026-09-16.json").exists())
