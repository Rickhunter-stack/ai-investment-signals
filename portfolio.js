(() => {
  ...
  function showRadar(){
    setActive('radar'); document.body.classList.remove('portfolio-mode');
    const main=document.querySelector('.main'); main.classList.remove('portfolio-active');
    window.dispatchEvent(new Event('resize'));
  }
  async function showPortfolio(){
    setActive('portfolio'); document.body.classList.add('portfolio-mode');
    const main=document.querySelector('.main'); main.classList.add('portfolio-active');
    let view=main.querySelector('.portfolio-view');
    if(!view){ view=document.createElement('div'); view.className='portfolio-view'; main.appendChild(view); }
    view.innerHTML='<div class="pf-empty">Chargement du portefeuille versionné…</div>';
    try{
      if(!cfg) cfg=await fetch('/data/portfolio.json?ts='+Date.now()).then(r=>{if(!r.ok)throw new Error('portfolio.json');return r.json()});
      if(!bench) bench=await fetch('/data/benchmark_history.json?ts='+Date.now()).then(r=>r.ok?r.json():({prices:[]}));
      renderPortfolio(view);
    }catch(e){view.innerHTML='<div class="pf-empty">Impossible de charger le portefeuille. Le prochain refresh GitHub Actions initialisera les données.</div>';}
  }

  function latestPrice(ticker){
    const r=(typeof ROWS!=='undefined'?ROWS:[]).find(x=>x.ticker===ticker); if(r&&r.close!=null)return Number(r.close);
    const h=(typeof HISTORY!=='undefined'&&HISTORY[ticker])||[]; return h.length?Number(h[h.length-1].close):null;
  }
  function holdingsNow(){
    const h={}, txs=[...(cfg.transactions||[])].sort((a,b)=>a.date.localeCompare(b.date)); let cash=Number(cfg.initial_capital);
    txs.forEach(t=>{const q=Number(t.quantity),p=Number(t.price),fees=Number(t.fees||0),sign=t.side==='sell'?-1:1; h[t.ticker]=(h[t.ticker]||0)+sign*q; cash+=t.side==='sell'?q*p-fees:-(q*p+fees);});
    const positions=Object.entries(h).filter(([,q])=>q>1e-10).map(([ticker,quantity])=>{const price=latestPrice(ticker);return {ticker,quantity,price,value:price==null?0:quantity*price};});
    return {cash,positions,value:cash+positions.reduce((s,p)=>s+p.value,0)};
  }
  function allDates(){
    const dates=new Set(); const inception=cfg.inception_date;
    Object.values(typeof HISTORY!=='undefined'?HISTORY:{}).forEach(arr=>arr.forEach(p=>{if(p.date>=inception)dates.add(p.date)}));
    (bench&&bench.prices||[]).forEach(p=>{if(p.date>=inception)dates.add(p.date)});
    (cfg.transactions||[]).forEach(t=>dates.add(t.date)); return [...dates].sort();
  }
  function priceAt(ticker,date){
    const arr=(typeof HISTORY!=='undefined'&&HISTORY[ticker])||[]; let v=null; for(const p of arr){if(p.date<=date)v=Number(p.close);else break;} return v;
  }
  function portfolioValueAt(date){
    const pos={}; let cash=Number(cfg.initial_capital);
    [...(cfg.transactions||[])].sort((a,b)=>a.date.localeCompare(b.date)).forEach(t=>{if(t.date>date)return;const q=Number(t.quantity),p=Number(t.price),fees=Number(t.fees||0),sgn=t.side==='sell'?-1:1;pos[t.ticker]=(pos[t.ticker]||0)+sgn*q;cash+=t.side==='sell'?q*p-fees:-(q*p+fees);});
    let value=cash; Object.entries(pos).forEach(([ticker,q])=>{if(q<=0)return;const p=priceAt(ticker,date);if(p!=null)value+=q*p;else{const tx=[...(cfg.transactions||[])].reverse().find(t=>t.ticker===ticker&&t.date<=date);if(tx)value+=q*Number(tx.price);}}); return value;
  }
  function universeIndex(date){
    const vals=[]; for(const ticker of cfg.universe_snapshot||[]){const arr=((typeof HISTORY!=='undefined'&&HISTORY[ticker])||[]).filter(p=>p.date>=cfg.inception_date&&p.date<=date);if(!arr.length)continue;const base=Number(arr[0].close),last=Number(arr[arr.length-1].close);if(base>0)vals.push(last/base);}
    return vals.length?100*vals.reduce((a,b)=>a+b,0)/vals.length:null;
  }
  function benchmarkIndex(date){
    const arr=(bench&&bench.prices||[]).filter(p=>p.date>=cfg.inception_date&&p.date<=date); if(!arr.length)return null; return 100*Number(arr[arr.length-1].value)/Number(arr[0].value);
  }
  ...
  function sharpe(series){if(series.length<20)return null;const r=[];for(let i=1;i<series.length;i++)if(series[i-1]>0)r.push(series[i]/series[i-1]-1);if(r.length<10)return null;const m=r.reduce((a,b)=>a+b,0)/r.length;const sd=Math.sqrt(r.reduce((s,x)=>s+(x-m)**2,0)/(r.length-1));return sd?m/sd*Math.sqrt(252):null;}

  function renderPortfolio(view){
    const now=holdingsNow(), perf=now.value/Number(cfg.initial_capital)-1;
    const bp=(bench&&bench.prices||[]).filter(p=>p.date>=cfg.inception_date), bperf=bp.length>1?Number(bp[bp.length-1].value)/Number(bp[0].value)-1:0, alpha=perf-bperf;
    const dates=allDates(), aisValues=dates.map(portfolioValueAt), aisIndex=aisValues.map(v=>100*v/Number(cfg.initial_capital)), uni=dates.map(universeIndex), bi=dates.map(benchmarkIndex);
    const dd=maxDrawdown(aisValues), sh=sharpe(aisValues), invested=now.positions.reduce((s,p)=>s+p.value,0), cashPct=now.value?now.cash/now.value:1;
    const positions=now.positions.map(p=>{const txs=(cfg.transactions||[]).filter(t=>t.ticker===p.ticker&&t.side==='buy');const cost=txs.reduce((s,t)=>s+Number(t.quantity)*Number(t.price)+Number(t.fees||0),0),qty=txs.reduce((s,t)=>s+Number(t.quantity),0),pru=qty?cost/qty:null,ret=pru&&p.price?p.price/pru-1:null;return {...p,pru,ret,weight:now.value?p.value/now.value:0};});
    ...
    view.innerHTML=`
      ...
      <div class="pf-grid">
        <section class="pf-card"><h3>100 investis depuis le départ</h3><div class="pf-chart"><canvas id="portfolioChart"></canvas></div><div class="pf-method">AIS = portefeuille réellement décidé · World = ${esc(cfg.benchmark.label)} · Univers = snapshot équipondéré des ${cfg.universe_snapshot.length} valeurs suivies au lancement.</div></section>
